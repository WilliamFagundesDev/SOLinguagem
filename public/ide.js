...
CodeMirror.defineMode("solinguagem", function() {
    return {
        // Agora com suporte a estado de comentário de bloco!
        startState: function() { return { depth: 0, inComment: false }; },
        token: function(stream, state) {
            
            // 1. Processamento de Comentários de Bloco /* ... */
            if (state.inComment) {
                while (!stream.eol()) {
                    if (stream.match("*/")) {
            ...
            // 2. Comentários de linha // e Abertura de Bloco /*
            ...
            // 4. Palavras, Funções e Identificadores
            const match = stream.match(/^[a-zA-Z_][a-zA-Z0-9_]*/);
            if (match) {
                const word = match[0];
                
                // ATUALIZADO: Todas as palavras do novo Motor Léxico integradas aqui!
            ...
            // 5. Operadores e Chaves (Brackets)
